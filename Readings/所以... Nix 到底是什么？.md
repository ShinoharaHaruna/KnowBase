...
这展示了另一个重要的 Nix 约定，即每个 `derivation` 都应创建一个输出文件夹结构，它匹配 [FHS](FHS.md)（Filesystem Hierarchy Standard，文件系统层次结构标准）。这只是意味着输出文件夹应该有熟悉的文件结构，例如用于二进制文件的 `bin`，用于库的 `lib`，用于头文件的 `include` 等等。
